...
async function fixCategories() {
  ...
  for (const article of articles) {
    ...
    const newCategory = await rssService.mapCategory('', article.title, article.synopsis || '', true);
    ...
    if (newCategory !== article.category) {
      ...
      updatedCount++;
      if (updatedCount % 50 === 0) console.log(`Updated ${updatedCount} article categories...`);
    }
  }
