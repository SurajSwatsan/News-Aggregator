...
async function testDeduplication() {
  ...
  try {
    ...
    let source = await prisma.source.findFirst({ where: { name: 'Test Source' } });
    if (!source) {
    ...
    const article1 = await prisma.article.create({
      data: {
        ...
        synopsis: 'A severe cyclonic storm has lashed Mumbai with heavy rains and strong winds, leading to widespread power cuts and traffic disruptions across the city.',
    ...
    console.log(`Created Article 1: ${article1.title} (Cluster: ${article1.clusterId})`);
    ...
    const article2 = await prisma.article.create({
      data: {
        title: 'Mumbai Storm: Heavy rain paralyzes city, power cut in many areas',
        synopsis: 'Mumbaikars faced a tough day as a massive storm caused heavy rainfall, paralyzing the city\'s transport and causing power outages in several neighborhoods.',
    ...
    console.log(`Created Article 2: ${article2.title} (Cluster: ${article2.clusterId})`);
    ...
    console.log(`Found ${cluster9999.length} article(s) for cluster 9999 in the feed.`);
    ...
    if (cluster9999.length === 1) {
      ...
      console.log(`❌ Failure: Expected 1 article, found ${cluster9999.length}.`);
    }
