...
export class AppService {
  ...
  async getPublicArticles(
    ...
    const clusterSizeMap = new Map(
      clusterCounts.map((c) => [c.clusterId, c._count._all]),
    );

    // Calculate Importance Score for each article
    const scoredArticles = articles.map((article) => {
      const clusterSize = article.clusterId
        ? clusterSizeMap.get(article.clusterId) || 1
        : 1;
      const sourcePriority = (article.source as any)?.priority || 0;

      // Recency Score: Linear decay over 48 hours (max 100 points)
      const hoursOld =
        (Date.now() - article.postedAt.getTime()) / (1000 * 60 * 60);
  ...
  async getArticleById(id: string) {
    ...
    if (article.clusterId !== null) {
      ...
      const otherPublishers = clusterArticles.filter(
        (a) => a.sourceId !== article.sourceId,
      );
      const samePublisher = clusterArticles.filter(
        (a) => a.sourceId === article.sourceId,
      );
    ...
    if (relatedArticles.length < 6) {
          ...
          for (let j = i + 1; j < significantKeywords.length; j++) {
            highRelQueries.push({
              AND: [
                {
                  title: {
                    contains: significantKeywords[i],
                    mode: 'insensitive' as const,
                  },
                },
                {
                  title: {
                    contains: significantKeywords[j],
                    mode: 'insensitive' as const,
                  },
                },
              ],
      ...
      if (relatedArticles.length < 5) {
        const searchTerms =
          significantKeywords.length > 0
            ? significantKeywords
            : keywords.slice(0, 2);
        if (searchTerms.length > 0) {
          const additional = await this.prisma.article.findMany({
            where: {
              id: { notIn: [id, ...relatedArticles.map((a) => a.id)] },
  ...
  async getTrendingArticles() {
    ...
    const trendingList = await Promise.all(
      articles.map(async (article) => {
        ...
        return {
          ...
          clusterSize,
          isSpike:
            recentViews > 5 && recentViews / Math.max(1, totalViews) > 0.4,
    ...
    return uniqueTrending.sort((a, b) => b.score - a.score).slice(0, 10);
  }

  private deDuplicate(
    articles: any[],
    sortBy: 'postedAt' | 'score' = 'postedAt',
  ): any[] {
    ...
    const deduplicatedClusters = Array.from(clusterMap.values())
      .map((clusterGroup) => {
        ...
        clusterGroup.sort((a, b) => {
          ...
          return (
            (b.importanceScore || b.score) - (a.importanceScore || a.score)
          );
        });
        ...
        if (clusterSize >= 4 && clusterGroup.length > 1) {
          ...
          if (
            ...
            (secondBest.importanceScore || 0) > 150
          ) {
